...
router.post('/register', async (req, res) => { // Made async
  try {
    ...
    if (allowedDomain && !email.endsWith(allowedDomain)) {
      return res.status(400).json({ error: `Registration is only allowed for ${allowedDomain} emails.` });
    }
    ...
    console.log(`New user created (in memory): ${email}, Hash: ${hashedPassword.substring(0, 10)}...`);
    const userId = 'dummy-user-id-from-db'; // Get this ID from your database response
    ...
    const token = jwt.sign(
        ...
        { expiresIn: process.env.JWT_EXPIRES_IN } // Expiration from .env
    );
...
router.post('/login', async (req, res) => { // Made async
  try {
    ...
    // --- Check 1: User exists ---
    if (!user) {
      return res.status(401).json({ error: 'Invalid credentials. User doesnt exist' }); // 401 = Unauthorized
    }
